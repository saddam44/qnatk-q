...
export class QnatkControllerService {
  ...
  async addNew<UserDTO = any>(baseModel: string, data: any, user: UserDTO) {
    const final_data = await this.sequelize.transaction(async (transaction) => {
  ...
  async executeAction<UserDTO = any>(
    ...
    const final_data = await this.sequelize.transaction(async (transaction) => {
      ...
      const model_instance = await this.qnatkService.findOneFormActionInfo(
        baseModel,
  ...
  async bulkExecuteAction<UserDTO = any>(
    ...
    const final_data = await this.sequelize.transaction(async (transaction) => {
      ...
      const model_instance = await this.qnatkService.findAllFormActionInfo(
        baseModel,
        validated_data.data.action,
        validated_data,
        transaction,
